...
const char *WINDOWTITLE = { "Project_4 - Chih-Hsiang Wang" };
...
void SphereTexture(float radius, int slices, int stacks);
...
Display( )
{
    ...
    glPushMatrix();
    glShadeModel(GL_SMOOTH);
    glTranslatef(3 * cos(2 * M_PI * Time), 6 * cos(2 * M_PI * Time), 6.);
    ...
    glTranslatef(-5 * cos(2 * M_PI * Time), 0., -5);
    SetMaterial(0.5, 0.5, 0.1, 50.);
    glutSolidTeapot(2);
    glPopMatrix();
    ...
    glTranslatef(3, 3, 0);
    glRotatef(90, 1, 0, 0);
    SetMaterial(1, 0.3, 0.7, 50.);
    glutSolidCone( 1, 5, 30, 30);
    glPopMatrix();
    
    //light0: blue spotlight at (0,0,0) toward -z
    SetSpotLight(GL_LIGHT0, 0, 0., 0, 0, 0., 10, 0, 0., 1.);
    glPushMatrix();
    glDisable(GL_LIGHTING);
    glColor3f(0, 0, 1);
    ...
    //light1: purple point light at (0,0,10)
    SetPointLight(GL_LIGHT1, 0., 0., 9., 0.2, 0, 0.2);
    glPushMatrix();
    glDisable(GL_LIGHTING);
    glColor3f(0.2, 0, 0.2);
    glTranslatef(0, 0., 10);
    ...
    //light2: white point light rotating
    SetPointLight(GL_LIGHT2, 8 * cos(2 * M_PI * Time), 5., 8 * sin(2 * M_PI * Time), 0., 0.7, 0);
    glPushMatrix();
    glDisable(GL_LIGHTING);
    glColor3f(1.0, 1.0, 1.0);
    glTranslatef(0., -5., 0.);
    glTranslatef(4 * cos(2 * M_PI * Time), 4., 8 * sin(2 * M_PI * Time));
    glutSolidSphere(0.3, 50, 50);
    glEnd();
    glEnable(GL_LIGHTING);
    glPopMatrix();
    ...
    glBindTexture(GL_TEXTURE_2D, tex0);
    SphereTexture(SPHERE_RADIUS, SPHERE_SLICES, SPHERE_STACKS);
    glEnable(GL_LIGHTING);
    glShadeModel(GL_SMOOTH);
    glColor3f(1, 1, 1);
    ...
    glDisable(GL_LIGHTING);
    glColor3f( 1., 1., 1. );
    DoRasterString( 5., 5., 0., "Project_4" );
...
unsigned char* BmpToTexture(char *filename, int *width, int *height) {
    ...
    fp = fopen("/Users/jerrywang/Documents/CS550_Computer_Graphics/HW/HW4/cs550_project_4/cs550_project_4/worldtex.bmp", "rb");   //path of the file
    if (fp == NULL) {
...
void SphereTexture(float radius, int slices, int stacks) {
    ...
    for (int ilat = 0; ilat < NumLats; ilat++) {
        ...
        for (int ilng = 0; ilng < NumLngs; ilng++) {
            ...
            p->nz = z;
            if (IfDistort) {
                p->s = (lng + M_PI) / (2.*M_PI) + Time/4/360;
                p->t = (lat + M_PI/2.) / M_PI + Time/4/360;
            }
            else {
                p->s = (lng + M_PI) / (2.*M_PI);
                p->t = (lat + M_PI/2.) / M_PI;
